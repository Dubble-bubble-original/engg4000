Feature: User post endpoints tests
  ...
  Scenario: Calling recentposts endpoint
    ...
    * def post4_access_key = response.post.access_key
    ...
    # Calling recentPosts endpoint
    Given path 'recentposts'
    And header token = auth_token
    When method post
    Then status 200
    And match response[0] contains read('../data/filter_userPosts_data.json').post4
    And match response[1] contains read('../data/filter_userPosts_data.json').post3
    And match response[2] contains read('../data/filter_userPosts_data.json').post2
    And match response[3] contains read('../data/filter_userPosts_data.json').post1
